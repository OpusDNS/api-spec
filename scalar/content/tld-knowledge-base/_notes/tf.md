...
[`GET /v1/tlds/tf`](/api-reference#tag/tld/GET/v1/tlds/{tld}) carries its own `type`,
...
The registrant is **not** part of a `.tf` transfer request: an inbound transfer keeps the
...
domain update — `.tf` supports a registrant change through
