...
| Property | Value |
| --- | --- |
| WHOIS Server | `whois.nic.wf` |
| RDAP Server | [rdap.nic.wf](https://rdap.nic.wf) |
...
curl "$OPUSDNS_API_BASE/v1/contacts/attribute-sets" \
  ...
  --data '{
    "label": "WF registrant - individual",
    "tld": "wf",
    "attributes": {
...
curl "$OPUSDNS_API_BASE/v1/contacts/attribute-sets" \
  ...
  --header "Content-Type: application/json" \
  --data '{
    "label": "WF registrant - association",
    "tld": "wf",
    "attributes": {
...
[`GET /v1/tlds/wf`](/api-reference#tag/tld/GET/v1/tlds/{tld}) carries its own `type`,
...
The registrant is **not** part of a `.wf` transfer request: an inbound transfer keeps the
...
domain update — `.wf` supports a registrant change through
